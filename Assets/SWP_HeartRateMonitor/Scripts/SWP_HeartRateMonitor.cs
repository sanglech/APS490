...
public class SWP_HeartRateMonitor : MonoBehaviour
{
	...
	public int BeatsPerMinute = 60; // Beats per minute.
	public static SWP_HeartRateMonitor Instance;
	...
	void Update()
	{
		BeatsPerSecond = 60f / BeatsPerMinute;
		//Instance = this;
		HeartBeatManager.Instance.HeartBeat = BeatsPerMinute;
		if(BeatsPerMinute<90)
		{
			SetHeartRateColour(NormalColour);
			HeartBeatManager.Instance.setColor(NormalColour);
		}
		if(BeatsPerMinute>=90&&BeatsPerMinute<120)
		{
			SetHeartRateColour(MediumColour);
			HeartBeatManager.Instance.setColor(MediumColour);
		}
		if(BeatsPerMinute>=120&&BeatsPerMinute<150)
		{
			SetHeartRateColour(BadColour);
			HeartBeatManager.Instance.setColor(BadColour);
		}
		if(BeatsPerMinute>=150&&BeatsPerMinute<182)
		{
			SetHeartRateColour(FlatlineColour);
			HeartBeatManager.Instance.setColor(FlatlineColour);
		}
		BlipOffset = new Vector3 (transform.position.x - (BlipMonitorWidth / 2), transform.position.y, transform.position.z);
		DisplayXEnd = BlipOffset.x + BlipMonitorWidth;
